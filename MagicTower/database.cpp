...
void DataBase::Connect() {         // Connect to SQLITE database
...
void DataBase::LoadMap(int num) {         // Load map data, num = 0 for new, others for old
    QSqlQuery query;
    QString str = "SELECT layer0, layer1, layer2, layer3, layer4, layer5, layer6, layer7, layer8, layer9 FROM map WHERE id = " + QString::number(num+1);
    query.exec(str);
    query.next();
...
void DataBase::LoadTools(int num) {         // Load tools data, num = 0 for new, others for old
    QSqlQuery query;
    QString str = "SELECT book, shield, sword FROM tools WHERE id = " + QString::number(num+1);
    query.exec(str);
    query.next();
...
void DataBase::LoadKeys(int num) {         // Load keys data, num = 0 for new, others for old
    QSqlQuery query;
    QString str = "SELECT red, blue, yellow FROM keys WHERE id = " + QString::number(num+1);
    query.exec(str);
    query.next();
...
void DataBase::SaveMap(int num) {         // Save map data
    QSqlQuery query;
    QString str = "UPDATE map SET layer0=?, layer1=?, layer2=?, layer3=?, layer4=?, layer5=?, layer6=?, layer7=?, layer8=?, layer9=? WHERE id=" + QString::number(num+1);
    query.prepare(str);
...
void DataBase::SaveTools(int num) {         // Save tools data
    QSqlQuery query;
    QString str = "UPDATE tools SET book=?, shield=?, sword=? WHERE id=" + QString::number(num+1);
    query.prepare(str);
...
void DataBase::SaveKeys(int num) {         // Save keys data
    QSqlQuery query;
    QString str = "UPDATE keys SET red=?, blue=?, yellow=? WHERE id=" + QString::number(num+1);
    query.prepare(str);
...
QString DataBase::layer(int num) {         // Construct layer string
    ...
    for (int i = 0; i < 14; i++)
        for (int j = 0; j < 14; j++) {
            if (map[j][i][num] < 10)
                str += "0";
            str += QString::number(map[j][i][num]);
            str += " ";
        }
    return str;
}
